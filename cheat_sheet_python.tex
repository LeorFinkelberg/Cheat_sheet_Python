...
Метод \texttt{transform} в самом простом случае применяет указанную функцию к каждому элементу вдоль указанного направления и возвращает результат преобразования, а в случае группировки для каждого элемента группы возвращается групповой аггрегат или результатом преобразования (причем для каждого столбца вычисляется свой аггрегат)
\begin{lstlisting}[
style = ironpython,
numbers = none
]
...
Другими словами, метод \verb|transform| на сгруппированном объекте для каждого элемента группы возвращает групповой аггрегат, а метод \verb*|apply| просто применяет указанную функцию к группе и возвращает результат для каждой группы
\begin{lstlisting}[
style = ironpython,
numbers = none
]
$ df.groupby("model_car_id")["year"].transfrom(np.count_nonzero)
0    5  # элемент 1970.0 был заменен мощностью его группы
1    4
2    5
3    4
4    4
...
$ df.groupby("model_car_id")["year"].apply(np.count_nonzero)
model_car_id
id_1    4
id_2    1
id_3    5
