...
\begin{itemize}
	\item \emph{Утиная типизация}: подход к типизации, по умолчанию принятый в Python с момента его возникнования. Утиная типизация это игнорирование фактического типа объекта и акцент на то, чтобы объект реализовывал методы с именами, сигнатурами и семантикой, требуемыми для конкретного применения. В Python это сводится в основном к тому, чтобы избегать использование функции \verb*|isinstance()| для проверки типа объекта \cite[\strbook{422}]{ramalho:python-2022}.
...
Реализации метода \verb*|__getitem__()| достаточно \cite[\strbook{414}]{ramalho:python-2022}:
\begin{itemize}
	\item для получения элементов по индексу,
	
	\item поддержке итерирования,
	
	\item оператора \verb*|in|
\end{itemize}
