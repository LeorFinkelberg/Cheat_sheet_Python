...
\emph{Аргументы функции ковариантны}: то есть если функция ожидает получить объект какого-то типа, то вместо этого типа можно передать и его дочерний тип. Но в функциях высшего порядка ситуация изменяется -- параметры функции становятся \emph{контрвариантными}, то есть ожидают получить указанный тип или его родительские типы.
